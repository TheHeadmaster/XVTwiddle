...
namespace XVTwiddle.Diagnostics
{
    /// <summary>
    ...
    public static class LogManager
    {
        ...
        /// <summary>
        /// Sets up the color theme for XVTwiddle.
        /// </summary>
        private static AnsiConsoleTheme XVTwiddleTheme { get; } = new AnsiConsoleTheme(
        ...
        public static void Initialize()
        {
            if (logPath.IsNullOrWhiteSpace())
            {
                ...
                logPath = Path.Combine(AppMeta.AppDataDirectory, "XVTwiddle", "Logs", $"{filename}.json");
            }
            if (!Directory.Exists(Path.Combine(AppMeta.AppDataDirectory, "XVTwiddle", "Logs")))
            {
                Directory.CreateDirectory(Path.Combine(AppMeta.AppDataDirectory, "XVTwiddle", "Logs"));
            }

            Log.Logger = new LoggerConfiguration()
                .MinimumLevel.Debug()
                .WriteTo.Console(outputTemplate: "[{Timestamp:HH:mm:ss} {Level}] {Message:lj}{NewLine}{Exception}", theme: XVTwiddleTheme)
